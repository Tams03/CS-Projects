...
def get_channel_data(channel_id, youtube):
    """Fetch channel data from YouTube API and add extra stats."""
    try:
        ...
        if 'items' in response and len(response['items']) > 0:
            ...
            data['Engagement Rate (%)'] = round((views / subscribers) * 100, 2) if subscribers > 0 else 0
            ...
            if categories:
                ...
                most_common_category = get_category_name(most_common_id, youtube)
            data['Most Common Category'] = most_common_category

            # Content frequency
            content_frequency = calculate_content_frequency(channel_id, youtube)
            data['Content Frequency'] = f"New content uploaded every {round(content_frequency,2)} days" if content_frequency else "Not enough data"

            # Most popular video
            most_popular_video = get_most_popular_video(channel_id, youtube)
            data['Most Popular Video'] = most_popular_video or "No popular video found"
...
def suggest_channel_based_on_others(channel_handles, youtube):
    ...
    input_channel_ids = []
    for handle in channel_handles:
        cid = get_channel_id(handle, youtube)
        if cid:
            input_channel_ids.append(cid)
